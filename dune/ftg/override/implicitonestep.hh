...
namespace Dune {
  namespace PDELab {
    ...
    public:
       ...
       */
      T apply (T time, T dt, TrlV& xold, TrlV& xnew,bool reuse_matrix=false)
      {
        ...
        for (unsigned r=1; r<=method->s(); ++r)
          {
            ...
            // solve stage
            try {
              pdesolver.apply(*x[r],reuse_matrix);
            }
            catch (...)
              {
       ...
       */
      template<typename F>
      T apply (T time, T dt, TrlV& xold, F& f, TrlV& xnew,bool reuse_matrix=false)
      {
        // do statistics
        OneStepMethodPartialResult step_result;
